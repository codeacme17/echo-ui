...
The command prints one JSON object after paginating all matching issues and open PRs. Start a Codex turn only when `hasWork` is `true`; pass the returned issue number and URL to a fresh Codex task instructed to read `loops/issue-dev-loop/LOOP.md` and the internal `SKILL.md` runbook by path.
