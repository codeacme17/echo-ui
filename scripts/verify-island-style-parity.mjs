...
const islandOutputRoot = resolve(repositoryRoot, 'docs', '.island', 'dist')
...
const fileForRequest = (requestUrl) => {
  ...
  assert.ok(
    ...
    'Parity requests must stay within the Nextra static output.',
  )

  return requestedPath
}

const islandFileForRequest = (requestUrl) => {
  const pathname = decodeURIComponent(new URL(requestUrl, 'http://localhost').pathname)
  ...
  assert.ok(
    requestedPath === islandOutputRoot || requestedPath.startsWith(`${islandOutputRoot}${sep}`),
    'Visual-reference requests must stay within the Island static output.',
  )
...
const islandServer = createServer(async (request, response) => {
  try {
    let filePath = islandFileForRequest(request.url ?? '/')
    try {
      const fileStats = await stat(filePath)
      if (fileStats.isDirectory()) filePath = resolve(filePath, 'index.html')
    } catch (error) {
...
const listen = (staticServer) =>
  new Promise((resolveListen, reject) => {
    staticServer.once('error', reject)
    staticServer.listen(0, '127.0.0.1', () => resolveListen(staticServer.address()))
  })

const closeServer = (staticServer) =>
...
const compareScreenshots = async (page, islandScreenshot, nextraScreenshot) =>
  page.evaluate(
    async ({ islandDataUrl, nextraDataUrl }) => {
      ...
      const [islandImage, nextraImage] = await Promise.all([
        loadImage(islandDataUrl),
        loadImage(nextraDataUrl),
      ])
      if (
        islandImage.naturalWidth !== nextraImage.naturalWidth ||
        islandImage.naturalHeight !== nextraImage.naturalHeight
      ) {
        throw new Error('Island and Nextra screenshots must use the same dimensions.')
      }

      const canvas = document.createElement('canvas')
      canvas.width = islandImage.naturalWidth
      canvas.height = islandImage.naturalHeight
      const context = canvas.getContext('2d', { willReadFrequently: true })
      context.drawImage(islandImage, 0, 0)
      const islandPixels = context.getImageData(0, 0, canvas.width, canvas.height).data
      context.clearRect(0, 0, canvas.width, canvas.height)
      context.drawImage(nextraImage, 0, 0)
      const nextraPixels = context.getImageData(0, 0, canvas.width, canvas.height).data

      let materiallyDifferentPixels = 0
      let totalDelta = 0
      for (let index = 0; index < islandPixels.length; index += 4) {
        const redDelta = Math.abs(islandPixels[index] - nextraPixels[index])
        const greenDelta = Math.abs(islandPixels[index + 1] - nextraPixels[index + 1])
        const blueDelta = Math.abs(islandPixels[index + 2] - nextraPixels[index + 2])
        const maximumDelta = Math.max(redDelta, greenDelta, blueDelta)
        totalDelta += redDelta + greenDelta + blueDelta
        if (maximumDelta > 32) materiallyDifferentPixels += 1
      }

      const pixelCount = canvas.width * canvas.height
      return {
        meanChannelDelta: totalDelta / (pixelCount * 3),
        mismatchRatio: materiallyDifferentPixels / pixelCount,
      }
    },
    {
      islandDataUrl: `data:image/png;base64,${islandScreenshot.toString('base64')}`,
      nextraDataUrl: `data:image/png;base64,${nextraScreenshot.toString('base64')}`,
    },
  )
...
const islandAddress = await listen(islandServer)
assert.ok(address && typeof address === 'object')
assert.ok(islandAddress && typeof islandAddress === 'object')
...
try {
  ...
  for (const profile of profiles) {
    ...
    const islandVisualPage = await activeContext.newPage()
    const nextraVisualPage = await activeContext.newPage()
    await Promise.all([
      islandVisualPage.goto(`http://127.0.0.1:${islandAddress.port}/en/`, {
        waitUntil: 'networkidle',
      }),
      nextraVisualPage.goto(
        `http://127.0.0.1:${address.port}${withBasePath('/en/')}`,
        { waitUntil: 'networkidle' },
      ),
    ])
    await Promise.all([
      islandVisualPage.evaluate(() => document.fonts.ready),
      nextraVisualPage.evaluate(() => document.fonts.ready),
    ])
    const [islandScreenshot, nextraScreenshot] = await Promise.all([
      islandVisualPage.screenshot({ animations: 'disabled' }),
      nextraVisualPage.screenshot({ animations: 'disabled' }),
    ])
    const comparisonPage = await activeContext.newPage()
    const visualDifference = await compareScreenshots(
      comparisonPage,
      islandScreenshot,
      nextraScreenshot,
    )
    console.log(
      `Visual ${profile.viewport.width}px ${profile.colorScheme}:`,
      visualDifference,
    )
    assert.ok(
      visualDifference.mismatchRatio <= 0.02,
      `${profile.viewport.width}px ${profile.colorScheme} Island/Nextra screenshots differ in more than 2% of pixels.`,
    )
    assert.ok(
      visualDifference.meanChannelDelta <= 2.5,
      `${profile.viewport.width}px ${profile.colorScheme} Island/Nextra screenshots exceed the color-delta budget.`,
    )
    await Promise.all([
      islandVisualPage.close(),
      nextraVisualPage.close(),
  ...
  await activeContext?.close()
  await browser?.close()
  await Promise.all([closeServer(server), closeServer(islandServer)])
}

console.log(
  'Nextra reproduces Island across 120 component, 8 localized home, and 4 live visual comparisons.',
)
